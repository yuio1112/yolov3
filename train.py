...
#   读入xml文件，并输出y_true
#---------------------------------------------------#
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):       #输入真实box位置，图像shape
    ...
    # 读出xy轴，读出长宽
    # 中心点(m,n,2)
    boxes_xy = (true_boxes[..., 0:2] + true_boxes[..., 2:4]) // 2       #根据box求xyhw
    boxes_wh = true_boxes[..., 2:4] - true_boxes[..., 0:2]
    # 计算比例
    true_boxes[..., 0:2] = boxes_xy/input_shape[:]      #归一到0-1
    ...
    # [1,9,2]
    anchors = np.expand_dims(anchors, 0)            #先验框的维度扩增，便于与xyhw计算
...
if __name__ == "__main__":
    ...
    checkpoint = ModelCheckpoint(log_dir + 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
        monitor='val_loss', save_weights_only=True, save_best_only=False, period=1)         #过两周期保存
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=2, verbose=1)        #学习率调整
    ...
    # model.load_weights("logs/ep039-loss0.184-val_loss0.158.h5")
